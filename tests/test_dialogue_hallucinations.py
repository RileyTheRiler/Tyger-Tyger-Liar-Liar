...
def dialogue_manager():
    ...
    # Load the deployed demo dialogue from data/dialogues
    # Note: We assume the script is running from repo root, so data/dialogues is accessible
    # or we construct path relative to this test file.
    data_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../data/dialogues'))

    # Check if file exists to avoid confusing errors
    if not os.path.exists(os.path.join(data_dir, "hallucination_demo.json")):
        pytest.fail(f"Demo scene not found at {data_dir}/hallucination_demo.json")

    success = dm.load_dialogue("hallucination_demo", data_dir)
...
def test_ambiguous_options_visibility(dialogue_manager):
    ...
    node = dm.nodes["start"]
    # Add an ambiguous choice dynamically to the loaded node
    node["choices"].append({
