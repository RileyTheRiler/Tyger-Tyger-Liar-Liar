...
def format_skill_result(result, printer=print):
    ...
    check_type = result.get("type", "white")  # Phase 7
    ...
    # Phase 7: Check type warning
    if not success:
        if check_type == "red":
            printer(f" {Colors.RED}⚠️  [RED CHECK - PERMANENT FAILURE] This cannot be retried.{Colors.RESET}")
        elif check_type == "white":
            printer(f" {Colors.WHITE}ℹ️  [WHITE CHECK - RETRYABLE] You may retry after improving your skill.{Colors.RESET}")

def format_skill_check_prompt(skill_name: str, difficulty: int, check_type: str = "white", printer=print):
    """Displays a pre-check prompt with visual warnings about check type."""
    if check_type == "red":
        printer(f"\n{Colors.RED}{Colors.BOLD}⚠️  RED CHECK ⚠️{Colors.RESET}")
        printer(f"{Colors.RED}This is a PERMANENT check. Failure cannot be undone.{Colors.RESET}")
    else:
        printer(f"\n{Colors.WHITE}ℹ️  WHITE CHECK{Colors.RESET}")
        printer(f"{Colors.WHITE}You may retry this check if you improve your skills.{Colors.RESET}")
    
    printer(f"\n{Colors.SKILL}[ {skill_name.upper()} vs DC {difficulty} ]{Colors.RESET}")
