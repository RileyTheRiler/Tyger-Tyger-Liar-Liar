...
class Item:
    ...
    def get_temperature_status(self) -> str:
        """Return a descriptive status for the item's temperature."""
        t = self.temperature
        if t <= 32: return "Freezing"
        if t <= 50: return "Cold"
        if t <= 80: return "Neutral" # Normal ambient
        if t <= 99: return "Warm"
        if t <= 101: return "Feverish"
        return "Overclocked"
...
class Evidence:
    def __init__(self, id: str, description: str, type: str, location: str = "unknown", collected_with: str = None, 
                 ...
                 case_id: str = "general", collected_from: str = "unknown"):
        self.id = id
        ...
        self.examined = False
        self.collected_from = collected_from

        # Phase 5 additions: Reliability & Aging
        self.is_reliable = True # The objective truth
        self.reliability_detected = True # If False, player hasn't realized it's fake/planted
        self.lens_interpretations: Dict[str, str] = {} # "believer": "...", "skeptic": "..."
        self.quality = 100.0 # 0-100
        self.degrades = False
    ...
    def to_dict(self):
        return {
            ...
            "lens_interpretations": self.lens_interpretations,
    ...
    def from_dict(cls, data):
        obj = cls(
            ...
            case_id=data.get("case_id", "general"),
            collected_from=data.get("collected_from", "unknown")
        )
        obj.tags = data.get("tags", [])
        obj.analysis_results = data.get("analysis_results", {})
        obj.examined = data.get("examined", False)
        
        # Phase 5 restore
        obj.is_reliable = data.get("is_reliable", True)
        obj.reliability_detected = data.get("reliability_detected", True)
        obj.lens_interpretations = data.get("lens_interpretations", {})
        obj.quality = data.get("quality", 100.0)
...
class InventoryManager:
    ...
    def add_evidence(self, evidence: Evidence):
        ...
        print(f"[Evidence] Collected: {evidence.id} ({evidence.case_id})")

    def load_documents(self, filepath: str):
        """Loads epistolary documents from a JSON file."""
        import os
        if os.path.exists(filepath):
            try:
                ...
                print(f"[Inventory] Error loading documents: {e}")
        else:
            print(f"[Inventory] Document file not found: {filepath}")
            self.document_database = {}

    def add_document(self, doc_id: str):
        """Unlocks a document for the player."""
        if not hasattr(self, "document_database"):
            self.document_database = {}
            
        doc = self.document_database.get(doc_id)
        if doc and doc not in self.documents:
            self.documents.append(doc)
            print(f"[DOCUMENT UNLOCKED] {doc.get('title', 'Unknown Document')}")
            return True
        return False
    ...
    def update_evidence_aging(self, hours_passed: float):
        """Reduces evidence quality over time for degradable items."""
        for ev in self.evidence_collection.values():
            if ev.degrades and ev.quality > 0:
                loss = ev.degradation_rate * hours_passed
                ev.quality = max(0, ev.quality - loss)
                if ev.quality < 50 and ev.quality + loss >= 50:
                    print(f"[DEGRADATION] Evidence '{ev.name}' is significantly degraded.")
                elif ev.quality == 0 and ev.quality + loss > 0:
                    print(f"[DEGRADATION] Evidence '{ev.name}' is now useless.")
    ...
    def get_item_by_name(self, name: str) -> Optional[Item]:
        for item in self.carried_items:
            if item.name.lower() == name.lower() or item.id == name:
                return item
        return None
