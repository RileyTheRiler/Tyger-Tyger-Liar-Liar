...
class NPC:
    ...
    def __init__(self, data: dict):
        ...
        # Week 12: Enhanced Relationship Tracking
        self.rapport = data.get("initial_rapport", 50)
        self.respect = data.get("initial_respect", 50)
        self.tags = data.get("tags", [])
        self.emotional_flags = data.get("emotional_flags", [])
        ...
        # Week 16: Integration fields
        self.is_integrated = data.get("is_integrated", False)
        self.integration_temperature = data.get("integration_temperature", 98.6)
        self.micro_pause_enabled = data.get("micro_pause_enabled", False)
    ...
    def modify_rapport(self, amount: int, reason: str = "") -> int:
        """Modify rapport level. Rapport affects dialogue warmth and cooperation."""
        self.rapport = max(0, min(100, self.rapport + amount))
        return self.rapport
    
    def modify_respect(self, amount: int, reason: str = "") -> int:
        """Modify respect level. Respect affects NPC deference and willingness to help."""
    ...
    def get_relationship_data(self) -> dict:
        """Get all relationship stats for this NPC."""
        return {
            "trust": self.trust,
            "fear": self.fear,
            ...
            "status": self.get_relationship_status(),
            "tags": self.tags.copy(),
            "emotional_flags": self.emotional_flags.copy(),
            "last_seen": self.last_seen
        }
    
    # Week 16: Integration Methods
    
    def mark_as_integrated(self, temperature: float = 99.3):
        """
        Mark this NPC as integrated by the Entity.
        Sets integration flags and temperature.
        """
        self.is_integrated = True
        self.integration_temperature = temperature
        self.micro_pause_enabled = True
        self.tags.append("integrated")
    
    def get_integration_clues(self, player_lens: str = "neutral", has_thermal: bool = False) -> List[str]:
        """
        Get detectable clues that this NPC is integrated.
        Returns list of clue descriptions.
        """
        ...
        # Thermal clue (requires thermal scanner)
        if has_thermal:
            clues.append(f"Body temperature: {self.integration_temperature}°F. Slightly elevated.")
        
        # Lens-specific clues
        if player_lens == "believer":
            clues.append("A faint shimmer around their head, like heat distortion.")
        elif player_lens == "skeptic":
            clues.append("Their story has inconsistencies. Small details that don't add up.")
        elif player_lens == "haunted":
            clues.append("They remind you of someone. Everyone does now.")
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "micro_pause_enabled": self.micro_pause_enabled
        }
    ...
    def restore_state(npc: 'NPC', state: dict):
        ...
        npc.rapport = state.get("rapport", npc.rapport)
        npc.respect = state.get("respect", npc.respect)
        npc.tags = state.get("tags", npc.tags)
        npc.emotional_flags = state.get("emotional_flags", npc.emotional_flags)
        npc.last_seen = state.get("last_seen", npc.last_seen)
        npc.flags = state.get("flags", npc.flags)
        
        # Week 16 fields
        npc.is_integrated = state.get("is_integrated", False)
        npc.integration_temperature = state.get("integration_temperature", 98.6)
        npc.micro_pause_enabled = state.get("micro_pause_enabled", False)
